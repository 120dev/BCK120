...
if [ -z "${MOUNT_SHARE_SCRIPT}" ]; then MOUNT_SHARE_SCRIPT="${VAR_BCK_CHROOT}/plugins/mount/mount_vincent.sh"; fi;
...
case "${1}" in
	umount)
		...
		auto_umount "10.0.0.50/test" "/BCK120-test"
	;;
	*)
		if [ ${umount_before} == "TRUE" ]; then ${MOUNT_SHARE_SCRIPT} umount; fi

		status 4 'On monte les partages :'; br
		auto_mount_univ "10.0.0.50" "/test" "/BCK120-test" "${IDENT_vincent}"
	;;
esac
